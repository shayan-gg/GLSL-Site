...
// SDFs
    ...
    {   
        ...
        return 2. * (texture(textTex, p / 16. + fract(vec2(char, 15. - floor(char / 16.)) / 16.)).w - 127. / 255.) +0.05;
    }
...
vec4 scene(vec3 position) {
    ...
    textPos.y += sin(time);
    float textScale = 0.2;
    //float char = floor(value.w*100.);
    //float distance3 = textSDF(textPos, textScale, charArr[0]);
